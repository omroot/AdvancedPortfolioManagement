...
def correlation2covariance(correlation_matrix: np.ndarray,
...
def build_true_matrix(number_blocks: int,
                      block_size: int,
                      block_correlation: float) -> Union[np.ndarray, np.ndarray]:
    ...
    mu = np.random.normal(standard_deviations, standard_deviations, covariance_matrix.shape[0]).reshape(-1, 1)
